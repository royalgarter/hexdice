...
*Version 1.2 introduces the option of larger R=8 map option and rules for random terrain generation.*

### **1\. Overview**

Hex Dice is a tactical board game where players command armies of dice, using their face values to determine their unit type, abilities, and movement. Played on a hexagonal map, players maneuver their forces, engage in deterministic combat, and aim to defeat all opponents or capture their base. Version 1.2 adds rules for playing on normal R=6 or a larger R=8 map with variable terrain.
...
- **Engaged Ranged Attack Override**: Units attacking from TOWER or MOUNTAIN terrain can perform ranged attacks even when an enemy unit is adjacent (engaged). This overrides the normal Archer restriction.
- **Line of Sight**: FOREST, TOWER, and MOUNTAIN hexes block line of sight for both ranged attacks and spell targeting. LAKE hexes remain transparent.
