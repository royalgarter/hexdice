...
```javascript
upgrades: {
    1: { atk: 0, def: 0, hp: 0, points: 0, perks: { tier1: 'A', tier2: null, tier3: null } },
...
A typical session in the Campaign Journey follows this recursive loop:

1.  **Preparation (The Camp):** The player allocates any earned **Devotion Points** and selects **Mastery Perks** if thresholds are met.
2.  **Deployment:** The player chooses which units to bring into the next level. Note that units eliminated in the previous level may still be recovering.
3.  **The Battle:** Combat uses the scaled HP and damage formulas. Victory is achieved by capturing the enemy base or annihilating all enemy forces.
4.  **Victory & Reward:** Upon success, the player earns **1 Devotion Point** and advances to the next level.
5.  **Scaling:** Every 10th level (The Crucible), the enemy army grows in size, testing the player's cumulative build choices.

*The journey continues until the player is defeated or reaches the ultimate milestone of Level 100.*