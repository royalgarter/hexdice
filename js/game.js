...
// http://localhost:1166/?autochess=true&auth_user=a
// http://localhost:1166/?autochess=true&auth_user=b
...
function alpineHexDiceTacticGame() { return {
	...
	toggleEmpiresMode() {
		this.empiresModeEnabled = !this.empiresModeEnabled;
		this.addLog(`👑 Empires Mode: ${this.empiresModeEnabled ? 'ENABLED' : 'DISABLED'}`);

		if (this.empiresModeEnabled) {
			this.assignEmpires();
		} else {
			this.players.forEach(p => p.empire = null);
		}
	},

	assignEmpires() {
		const empireNames = Object.keys(this.EMPIRES);
		// Shuffle empire names
		for (let i = empireNames.length - 1; i > 0; i--) {
			const j = Math.floor(random() * (i + 1));
			[empireNames[i], empireNames[j]] = [empireNames[j], empireNames[i]];
		}

		this.players.forEach((player, i) => {
			player.empire = empireNames[i % empireNames.length];
			this.addLog(`P${i + 1} assigned to the ${player.empire} Empire!`);
		});
	},
	...
	handleAuthoritativeState(state) {
		...
		if (state.hexes) {
			state.hexes.forEach(sh => {
				const lh = this.hexes[sh.id];
				if (lh) {
					// Update terrain if changed (e.g. after nextRound generates new roulette terrain)
					if (sh.terrainType && sh.terrainType !== lh.terrainType) {
						this.setTerrainType(lh, sh.terrainType);
					}

					lh.unitId = sh.unitId;
					lh.unit = null;
					// Look up unit from player dice to maintain object identity,
					// so combat updates via unitMap reflect in hex display
					if (sh.unitId) {
						for (const p of this.players) {
							const match = p.dice.find(d => d.id === sh.unitId);
							if (match) {
								lh.unit = match;
								if (match.hexId && match.hexId !== lh.id) {
									match.hexId = lh.id;
								}
								break;
							}
	...
	async initOnlineGame() {
		...
		// Assign names, IDs, and force correct configuration
		if (this.online.isHost) {
			if (this.players[0]) {
				Object.assign(this.players[0], PLAYER_CONFIG[0]);
				this.players[0].name = this.auth.user.name;
				this.players[0].id = this.auth.user._key;
			}
			if (this.players[1]) {
				Object.assign(this.players[1], PLAYER_CONFIG[1]);
	...
	getRadius(link=location) {
		...
		if (typeof document !== 'undefined' && document.querySelector) {
			...
			if (dom && dom.classList && typeof dom.classList[Symbol.iterator] === 'function') {
				if (!dom.classList.contains(cls)) {
					const existing = [...dom.classList].find(x => x.includes('radius-'));
					if (existing) dom.classList.remove(existing);
	...
	rollInitialDice(playerId) {
		...
		if (this.empiresModeEnabled && !player.empire) {
			this.assignEmpires();
		}
		...
		for (let i = 0; i < this.rules.dicePerPlayer; i++) {
			...
			const baseStats = UNIT_STATS[roll];
			...
			const die = {
				...
				playerId: playerId,
				value: roll,
				...baseStats, // Spread initial stats
			...
			die.effectiveArmor = die.currentArmor;
			die.spriteUrl = this.getUnitSpriteUrl(die);
	...
	performReroll() {
		...
		this.diceToReroll.forEach(diceIndex => {
			...
			player.dice[diceIndex].effectiveArmor = player.dice[diceIndex].currentArmor;
	...
	performOracleTransmute(oracleHexId, targetHexId, state) {
		...
		if (reserveDie) {
			...
			reserveDie.effectiveArmor = reserveDie.currentArmor;
	...
	handleCombat(attackerHexId, defenderHexId, combatType, state) { // combatType: 'MELEE', 'RANGED_ATTACK', 'COMMAND_CONQUER'
		...
		if (this.empiresModeEnabled && this.players[defenderUnit.playerId].empire === 'Japan' && defenderUnit.value === 4) {
			...
			if (dodgeRoll <= 2) {
				...
				this.addLog(`💨 Shinobi Dodge! ${this.logUnit(defenderUnit)} (D${dodgeRoll}) evaded the attack.`, state);
				attackerUnit.skirmishBuff = 0;
				this.recordAction('COMBAT_DODGE', { fromHex: attackerHexId, toHex: defenderHexId, diceValue: dodgeRoll });
				return;
			}
		}
	...
	handleNomadicRaidSuccess(attackerHexId, defenderHexId, state) {
		const attackerUnit = this.getUnitOnHex(attackerHexId, state);
		const defenderHex = this.getHex(defenderHexId, state);
		const attackerHex = this.getHex(attackerHexId, state);
		
		if (!state) window?.AudioManager?.playSfx('horse');
		this.addLog(`🐎 Nomadic Raid! Choose a destination adjacent to the target.`, state);

		if (!state) {
			// First, move to the target hex (standard melee win behavior)
			this.move(attackerUnit, attackerHex, defenderHex);
			
			this.actionMode = 'SKIRMISH_POST_MOVE';
			this.selectedUnitHexId = defenderHexId; // Attacker is now here
			const neighbors = this.getNeighbors(defenderHex);
			this.validMoves = neighbors
				.filter(n => !this.getUnitOnHex(n.id) || n.id === defenderHexId)
				.map(n => n.id);
			if (!this.validMoves.includes(defenderHexId)) this.validMoves.push(defenderHexId);
		}
	},
	...
	cloneState(game) { // Very low performance
		const src = game || this;
		let data = JSON.parse(JSON.stringify(src.$data || src));
